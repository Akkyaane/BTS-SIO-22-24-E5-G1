    ...
    if (isset($_POST['submit'])) {
        ...
        if ($uploadOk == true) {
            if (empty($expenseSheet[':kn']) && empty($expenseSheet[':te']) && empty($expenseSheet[':nn']) && empty($expenseSheet[':ae']) && empty($expenseSheet[':fe']) && empty($expenseSheet[':oe'])) {
                ...
                $sql = 'UPDATE expensesheets SET user_id=:ui, receipts_id=:ri, request_date=:rd, start_date=:sd, end_date=:ed, transport_category=:tc, kilometers_number=:kn, transport_expense=:te, nights_number=:nn, accommodation_expense=:ae, food_expense=:fe, other_expense=:oe, message=:m, total_amount=:ta WHERE id=:id';
                $request = $dbConnect->prepare($sql);
